...
#
post_mcmc = read.csv('mcmc/network/mcmc_y_quantiles.csv')
post_mcmc$X = NULL
names(post_mcmc) = c('X','Mean', 'qlow', 'qhigh', 'med')

#Loading of cached data
dir_name_mcmc = 'runs/network'
y_preds_MFGauss = read.csv(file.path(dir_name_mcmc,'y_preds_VIGAUSS.csv'), header=FALSE)
y_preds_MFTM = read.csv(file.path(dir_name_mcmc,'y_preds_VIMLTS.csv'), header=FALSE)
xx = read.csv(file.path(dir_name_mcmc,'xx.csv'), header=FALSE)
dim(xx)
xx = xx[,1]

PDF = FALSE
#load(file.path(dir_name, 'loss_hist_1.rda'))
load(file.path(dir_name, 'samples_5.rda'))
...
### Loss
#plot(loss_hist, type='l', cex.main=0.6,main=dir_name, 
#     sub = paste0('k_hat ', round(k_hat,3)), 
#     col='skyblue', ylim=c(20, 100))
...
plot(x, y_med, main=dir_name, cex.main=0.8,ylim=c(-2.5,3), ylab='y', type='l', col='skyblue', lwd=2, las=1)
for (t in 1:20) {
