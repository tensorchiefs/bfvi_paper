###### Notes #####
# master for all experiments in the BF-VI paper
rm(T)
library(keras)
library(tensorflow)
...
if (FALSE){
  ...
  DATA = 'MELA'   #2-D regression on simulated P=2 predictors
  number_epochs = 15L
...
print(paste0('Starting Experiment: ', EXP_NAME))
NUM_REP = 5
RUN_DIR = 'runs'
...
T_INIT = 500 #Only relevant for ML initialisation
if (DATA == 'MELA') {
  ML_INIT = TRUE #Initialise the paramters with ML Solution
} else {
  ML_INIT = FALSE 
}

T_SAMPLES = 5000L #sampels after training
w_min =  -10 #Minima value of the start variational distibution
w_max =  10

if (DATA == 'MELA'){
  w_min =  -1 #Minima valuel of the start variational distibution
  w_max =  1
}
...
if(DATA == '8SCHOOLS_CP' || DATA == '1SCHOOLS_CP') {
  library(rstan)
  data <- read_rdump("data/eight_schools.data.R")
  ...
  get_log_prior = function(w){
    ...
    return(
      tfd_normal(0,5.)$log_prob(mu) + #mu 
        #The log(2.0) term is -log(1-cdf(0)) or using the effect that chauchy is symetrical simple multicly pdf*2
        tfd_cauchy(0.,5.)$log_prob(tau) + log(2.0) + #tau (added 26.7.2022 the log(2.0) term )
...
# MELA  --------
if(DATA == 'MELA') {  
  #trainData <- read.csv("~/Dropbox/__ZHAW/__Projekte_Post_ZHAH/shared_Oliver_Beate/tmvi/Ivonne_24_Sep/alter_outcome_data/trainData.csv")
  ...
  x_r = 1.*trainData$standAge
  y_r = 1.*trainData$target
  
  y = tf$reshape(y_r, shape=c(N, 1L))
  x = tf$reshape(tf$Variable(x_r), shape=c(N,P))
#  plot(x_r, y_r)
  
  g = glm(y_r ~ x_r, family = 'binomial')
  print(g)
  w_mu_init = tf$reshape(tf$Variable(c(g$coefficients[2], g$coefficients[1]), dtype='float32'), shape=c(2L,1L))
  
  w_sd_init_r = 5*summary(g)[["coefficients"]][,2]
  w_sd_init = tf$reshape(tf$Variable(w_sd_init_r, dtype='float32'), shape=c(2L,1L))
  
  #The prior and likelihood for the specific problem
  get_LL <- function(w, x, y, T) {
    #Determination of the likelihood
    slope = tf$slice(w,c(0L,0L),c(T,1L))
    intercept = tf$slice(w,c(0L,1L),c(T,1L)) #/ 50. # Adding constans (the GD way) 
    #Pi should be in the range ]0,1[ --> Adding constans (the GD way) 
    #pi = 1e-5 + 0.01 * tf$math$sigmoid(intercept + slope * tf$transpose(x)) 
    pi = tf$math$sigmoid(intercept + slope * tf$transpose(x)) 
    y_rep = tf$transpose(tf$tile(y, c(1L,T)))
    return (y_rep*log(pi) + (1L-y_rep)*log(1.-pi))
  }
  
  get_log_prior = function(w){
    slope = tf$slice(w,c(0L,0L),c(T,1L))
    intercept = tf$slice(w,c(0L,1L),c(T,1L)) 
    return(
        tfd_normal(0.,1.)$log_prob(intercept) + 
        tfd_normal(0.,1.)$log_prob(slope))
  } 
}
## END MELA
...
if (DATA == '2D') {
  ...
  if (FALSE){
    ...
    save(x_r,y_r, file='data/2D.rdata')
    evidence = -12.62
  }
  load('data/2D.rdata')  
  x_r
...
for (rep in 1:NUM_REP) {
  print(paste0('Starting repetition  :  ', rep))
...
a_z_prime = tf$Variable(rep(0.,D), shape=c(D))
b_z = tf$Variable(rep(0.,D), shape=c(D))
if (method == 'F1F2' || method == 'SigmoidF2'){
  #zss = tf$math$sigmoid(zs)
  base_dist=tfp$distributions$Normal(loc=0.,scale=1)
  #plot(zs, as.numeric(base_dist$prob(zss)), type='l', ylab='density', main='Base distribution')
} else if (method == 'TruncF2'){
...
loss = function(a_z_prime, b_z, theta_1_prime, maf,  method, return_w = FALSE){
  theta_1 = to_theta(theta_1_prime)
  ...
  if (method == 'F1F2') {
    a_z =  tf$math$softplus(a_z_prime)
    z_lin = a_z * z  + b_z
    z_bp = tf$math$sigmoid(z_lin)
  } else if (method == 'TruncF2') {
    z_bp = z
  } else if (method == 'SigmoidF2') {
    stop("Not implemented yet")
  } 
  w = eval_hD(theta_1, z1 = z_bp[,1,drop=FALSE], theta_2_D = theta_2_D, z_2_D = z_bp[,2:D,drop=FALSE])
  ...
  # In case of a composed triangular flow like F1F2 z_i->lin_i->sigmoid_i->Bern_i the logs of 
  # the Jacobians can be added. We start with the Bernstein Flow
  #----- Bernstein Part
  J11 = eval_h1_dash(to_theta(theta_1_prime), z_bp[,1,drop=FALSE])
  #Jii = d h_i /d z_i = \sum_m (theta_(m+1) -  theta_(m)) * Be_
  #See https://arxiv.org/abs/1508.06749 page 12
  BE = tf$reshape(beta_dists_for_h_dash$prob(
    tf$reshape(z_bp[,2:D,drop=FALSE], shape = c(T*(D-1L),1L))
  ), c(T,D-1L,M-1L))
  ...
  det_bp = log(J11) + tf$reduce_sum(log(J_ii),axis=1L) 
  
  log_det =  det_bp
  #----- Sigmoid Part
  if (method == 'F1F2' || method == 'SigmoidF2') {
    det_sig = tf$reduce_sum(
        tf$math$log_sigmoid(z_lin) + tf$math$log(1. - tf$math$sigmoid(z_lin)), #TODO second term has numerical potential
        axis=1L) 
    log_det = log_det + det_sig
  }
  #----- Linear Part
  if (method == 'F1F2') {
    log_det = log_det + tf$reduce_sum(tf$math$log(a_z))
  }
...
##### Gradient Descent ######
#LF(theta_1_prime, maf)
loss(a_z_prime, b_z, theta_1_prime, maf, method)
LF = tf_function(loss)
#LF = loss ######### TODO !!!! Please comment ou again
loss_hist = NULL
cubo_1_hist = NULL
for (i in 1:number_epochs){
  ...
  with(tf$GradientTape() %as% t, {
    loss_ = LF(a_z_prime, b_z, theta_1_prime, maf, method)
  })
  w_maf = maf$trainable_variables
  pars = c(theta_1_prime, w_maf)
  grads = t$gradient(loss_, pars)
...
if (DATA == 'MELA') {#Problem with Memory for 50k samples, splitting
  T_OLD = T
  T = T_SAMPLES
  samples = loss(a_z_prime, b_z, theta_1_prime, maf, method, return_w = TRUE)
  for (i in 1:10){
    samples = append(samples, loss(a_z_prime, b_z, theta_1_prime, maf, method, return_w = TRUE))
  }
  ################ Saving ########
  if(SAVE){
    save(loss_hist, file = file.path(EXP_DIR, paste0('loss_hist_',rep,'.rda')))
    save(samples, file = file.path(EXP_DIR, paste0('samples_',rep,'.rda')))
  }
} else{
  T_OLD = T
  T = T_SAMPLES
